...
class ThreadUrl(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            #json= '{ "key1": ' + str(count) + '}'
            json = '{ "key1": "'+str(mean)+'" , "key2": "'+str(std)+'" ,"key3": "'+str(Lenght_of_History)+'" ,"key4": "'+str(Number_of_Shots)+'" ,"key5": "'+str(len(data)) +'"}'
		...
		for i in range(Lenght_of_History, len(data)):
			if data.Buy[i] == 1:
				...
				c = http.client.HTTPSConnection(
				    "x3utnhvsoe.execute-api.us-east-1.amazonaws.com")
				json = '{ "key1": "'+str(mean)+'" , "key2": "'+str(std)+'" ,"key3": "'+str(Lenght_of_History)+'" ,"key4": "'+str(Number_of_Shots)+'" ,"key5": "'+str(len(data)) +'"}'
				c.request("POST", "/default/VaR", json)
				#print("Sent to lambda")
				response = c.getresponse()
				#print("Got the Response")
				res = response.read().decode('utf-8')	
				#print("Here suppose to return var95% + var 99%")
				print(res)
				return doRender('Result.html',
						{'note': res,
						'note2': str(time.time()-start ),
						'data': 0.95,
